...
登録サイトの記事カードから「保存する」を選ぶ場合は、記事 URL を `?url=...` としてこの画面へ渡す。画面を開いた時点で URL 入力欄へ初期値を反映し、その後は既存の抽出・確認・保存フローを利用する。
...
翻訳ボタンの下には、翻訳すると再抽出できなくなる旨を操作前から表示する。翻訳済みの記事は再抽出できない。別の記事 URL を抽出した場合は翻訳状態をリセットする。
...
Web アプリから `md-extractor` への Bearer token 認証が失敗した場合、`md-extractor` はサービス間の認証エラーとして `401` を返す。Web アプリの利用者が修正できる認証エラーではないため、記事抽出 API と記事翻訳 API は一般的なサーバエラーへ変換して `500` を返す。利用者自身の未認証は、従来どおり `401` として扱う。
