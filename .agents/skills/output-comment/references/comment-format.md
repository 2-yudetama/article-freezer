...
6. 標準入力や heredoc 経由の投稿で `gh auth login` や `GH_TOKEN` を要求するエラーになった場合は、認証破損と即断せず、短い `--body "..."` で投稿できるかを切り分ける
7. 長文 Output が標準入力経由で失敗する場合は、本文を `--body` で渡せる範囲に整形するか、Manager に戻して投稿方法を切り替える。投稿済みか不明な場合は既存コメントを確認し、重複投稿を避ける
8. 投稿 URL または投稿結果を記録する
